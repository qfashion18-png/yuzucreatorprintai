...
export class MockAiDesignAssistant implements AiDesignAssistant {
  async recommendProducts(_input: RecommendProductsInput): Promise<ProductRecommendation[]> {
    return [
      {
        productSlug: "die-cut-stickers",
        reason: "Stickers are the easiest first touchpoint for creator drops and unboxing extras.",
        priority: 1,
      },
      {
        productSlug: "qr-creator-cards",
        reason: "QR cards turn in-person moments into follows, listens, signups, and shop visits.",
        priority: 2,
      },
      {
        productSlug: "thank-you-insert-cards",
        reason: "Insert cards add a direct CTA to every shipment without changing the product.",
        priority: 3,
      },
    ];
  }
  ...
  async runPreflightNarration(input: PreflightResult): Promise<string> {
    ...
    const issueText = input.warnings.map((warning) => warning.message.toLowerCase()).join(" ");
