...
import { getProductBySlug, getTemplatesByProduct } from "@creator-print-ai/core";
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <section className="grid gap-8 rounded border border-slate-200 bg-white p-6 shadow-sm lg:grid-cols-[1fr_420px]">
        <div>
          <p className="text-sm font-black uppercase text-[#007f88]">{product.category}</p>
          <h1 className="mt-2 text-4xl font-black tracking-[0]">{product.name}</h1>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {product.popularUseCases.map((useCase) => (
              <span key={useCase} className="rounded bg-[#ecfeff] px-3 py-1 text-xs font-bold text-[#007f88]">
                {useCase}
              </span>
            ))}
          </div>
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link href="/templates" className="inline-flex items-center gap-2 rounded border border-slate-200 bg-white px-5 py-3 text-sm font-black">
              Choose template
            </Link>
          </div>
        </div>
        <aside className="rounded bg-[#f4fbff] p-5">
          <dl className="grid gap-4 text-sm">
            <div>
              <dt className="font-bold text-slate-500">Starting price</dt>
              <dd className="mt-1 text-2xl font-black">{formatMoney(product.startingPriceCents)}</dd>
            </div>
            <div>
      ...
      <section className="mt-10">
        <h2 className="text-2xl font-black tracking-[0]">Templates for {product.name}</h2>
